...
export const ProjectAmendmentsModal: React.FC<ProjectAmendmentsModalProps> = ({ project, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-sapv-blue-light rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        <header className="flex justify-between items-center p-4 border-b border-sapv-gray-dark">
          <div>
            <h2 className="text-2xl font-bold text-sapv-gray-light">Gerenciar Emendas</h2>
            <p className="text-sm text-sapv-gray">{project.title}</p>
          </div>
        ...
        <main className="flex-grow p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <section>
            ...
            <div className="space-y-3 pr-2 max-h-[calc(80vh-200px)] overflow-y-auto">
              {project.amendments && project.amendments.length > 0 ? (
                project.amendments.map(amend => (
                  <div key={amend.id} className="bg-sapv-blue-dark p-3 rounded-md">
          ...
          <section className="border-t md:border-t-0 md:border-l border-sapv-gray-dark pt-6 md:pt-0 md:pl-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label htmlFor="amend-title" className="block text-sm font-medium">Título da Emenda</label>
                <input id="amend-title" type="text" value={title} 
                       onChange={(e) => { setTitle(e.target.value); clearError('title'); }} 
                       className={`mt-1 w-full px-3 py-2 text-sapv-gray-light bg-sapv-blue-dark border rounded-md focus:ring-blue-500 focus:border-blue-500 ${errors.title ? 'border-red-500' : 'border-sapv-gray-dark'}`} 
                />
                {errors.title && <p className="text-red-400 text-xs mt-1">{errors.title}</p>}
              </div>
              <div>
                <label htmlFor="amend-author" className="block text-sm font-medium">Autor</label>
                <select id="amend-author" value={authorId} 
                        onChange={e => { setAuthorId(e.target.value); clearError('authorId'); }} 
                        className={`mt-1 w-full px-3 py-2 text-sapv-gray-light bg-sapv-blue-dark border rounded-md focus:ring-blue-500 focus:border-blue-500 ${errors.authorId ? 'border-red-500' : 'border-sapv-gray-dark'}`}
                >
              ...
              <div>
                <label htmlFor="amend-desc" className="block text-sm font-medium">Texto da Emenda</label>
                <textarea id="amend-desc" value={description} 
                          onChange={(e) => { setDescription(e.target.value); clearError('description'); }} 
                          rows={4} 
                          className={`mt-1 w-full px-3 py-2 text-sapv-gray-light bg-sapv-blue-dark border rounded-md focus:ring-blue-500 focus:border-blue-500 ${errors.description ? 'border-red-500' : 'border-sapv-gray-dark'}`} 
                />
