...
const DADOS_VEREADORES = [
  { id: 1, nome: 'PRESIDENTE DA CÂMARA', partido: 'PP', cargo: 'PRESIDENTE' },
  { id: 2, nome: 'FRANCIS DE GINALDO', partido: 'MDB', cargo: 'VICE' },
  { id: 3, nome: 'MANOEL DO POSTO', partido: 'PP', cargo: '1º SEC' },
  { id: 4, nome: 'DEL DO MERCADINHO', partido: 'PSDB', cargo: '2º SEC' },
  { id: 5, nome: 'DANIEL MIGUEL', partido: 'PSDB', cargo: '' },
  { id: 6, nome: 'LÊDO', partido: 'PSDB', cargo: '' },
  { id: 7, nome: 'IRMÃO BETO', partido: 'PP', cargo: '' },
  { id: 8, nome: 'SARGENTO VAL', partido: 'MDB', cargo: '' },
  { id: 9, nome: 'JULIANA VIDAL', partido: 'PP', cargo: '' },
  { id: 10, nome: 'JOÃO OLÍMPIO', partido: 'PSB', cargo: '' },
  { id: 11, nome: 'JOÃO SUFOCO', partido: 'UB', cargo: '' },
  { id: 12, nome: 'CHICO DO POVO', partido: 'MDB', cargo: '' },
];

const CardVereador = ({ vereador, fase }) => {
  ...
  if (vereador.presente) {
    ...
    if (fase === 'votacao') {
        ...
        } else if (vereador.voto === 'abstencao') {
  ...
  return (
    <div className={`flex flex-col rounded-xl border-2 ${corBorda} bg-gray-900/80 backdrop-blur-sm transition-all duration-300 ${sombra} h-full overflow-hidden`}>
      ...
      <div className="flex-1 flex items-center px-3 gap-3 min-h-0 py-1">
        ...
        {/* Bloco de Texto (Direita) - Alinhado à Esquerda e Centralizado Verticalmente */}
        <div className="flex-1 min-w-0 h-full flex flex-col justify-center items-start text-left">
            
            {/* Cargo - Slot reservado para manter altura constante */}
...
const ResumoVotacao = ({ vereadores, fase }) => {
  ...
  const nao = vereadores.filter(v => v.voto === 'nao').length;
  const abs = vereadores.filter(v => v.voto === 'abstencao').length;
...
export default function App() {
  const [vereadores, setVereadores] = useState(
    DADOS_VEREADORES.map(v => ({ ...v, presente: false, voto: null }))
  );
  const [fase, setFase] = useState('fechada');
  const [materiaTitulo, setMateriaTitulo] = useState('PROJETO DE LEI Nº 001/2025');
  const [materiaTexto, setMateriaTexto] = useState('Dispõe sobre a obrigatoriedade da instalação de painéis solares em prédios públicos e dá outras providências.');
  ...
  const togglePresenca = (id) => {
    setVereadores(prev => prev.map(v => v.id === id ? { ...v, presente: !v.presente, voto: !v.presente ? v.voto : null } : v));
  };

  const computarVoto = (id, tipoVoto) => {
    setVereadores(prev => prev.map(v => v.id === id && v.presente ? { ...v, voto: tipoVoto } : v));
  };

  const resetarVotos = () => setVereadores(prev => prev.map(v => ({ ...v, voto: null })));
  
  const mudarFase = (novaFase) => {
    if (novaFase === 'chamada') resetarVotos();
    setFase(novaFase);
  };
      ...
      <div className="flex-1 flex flex-col h-full p-2 md:p-4 bg-gradient-to-br from-gray-900 via-black to-gray-900 relative z-0">
        ...
        {fase !== 'fechada' && (
            <div className="mb-2 flex gap-2 h-24 md:h-28 flex-shrink-0 animate-in fade-in slide-in-from-top-2 duration-500">
                ...
                <div className={`w-28 rounded-xl flex flex-col items-center justify-center border-2 ${fase === 'votacao' ? 'border-red-600 bg-red-900/10' : 'border-gray-700 bg-gray-800/20'}`}>
                    {fase === 'votacao' ? (
                        <>
                            <Play size={28} className="text-red-500 mb-1 animate-pulse" />
                            <span className="text-red-500 font-black text-center text-xs animate-pulse leading-tight">VOTAÇÃO<br/>ABERTA</span>
                        </>
                    ) : fase === 'chamada' ? (
                        <>
                            <Users size={28} className="text-yellow-500 mb-1" />
                            <span className="text-yellow-500 font-black text-center text-xs">CHAMADA</span>
                        </>
                    ) : (
                        <>
                            <FileText size={28} className="text-blue-500 mb-1" />
                            <span className="text-blue-500 font-black text-center text-xs">LEITURA</span>
                        </>
                    )}
                </div>
            </div>
        ...
        <div className="flex-1 min-h-0 overflow-hidden pb-1">
            <div className="grid grid-cols-3 lg:grid-cols-4 gap-2 h-full content-stretch">
                {vereadores.map(ver => (
                    <CardVereador key={ver.id} vereador={ver} fase={fase} />
                ))}
            </div>
        </div>
      ...
      <div className={`fixed inset-y-0 right-0 w-96 bg-gray-900 border-l border-gray-700 shadow-2xl transform transition-transform duration-300 ease-in-out z-50 flex flex-col ${menuAberto ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-4 bg-gray-800 border-b border-gray-700 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2">
                ...
                <h2 className="font-bold text-white tracking-wide">MESA OPERADORA</h2>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            <section>
                ...
                <input 
                    type="text" 
                    value={numSessao} 
                    onChange={(e) => setNumSessao(e.target.value)}
                    className="w-full bg-black/30 border border-gray-600 rounded p-2 text-xs text-white focus:border-blue-500 outline-none"
                />
            </section>
            ...
            <section>
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                     <button onClick={() => mudarFase('fechada')} className={`p-2 rounded text-xs font-bold flex items-center justify-center gap-2 col-span-2 ${fase === 'fechada' ? 'bg-gray-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                        <Power size={14} /> MODO DE ESPERA / FECHADO
                    </button>
                </div>
                <div className="grid grid-cols-3 gap-2">
                    <button onClick={() => mudarFase('leitura')} className={`p-2 rounded text-xs font-bold flex flex-col items-center gap-1 ${fase === 'leitura' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                        <Mic size={16} /> LEITURA
                    </button>
                    <button onClick={() => mudarFase('chamada')} className={`p-2 rounded text-xs font-bold flex flex-col items-center gap-1 ${fase === 'chamada' ? 'bg-yellow-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                        <Users size={16} /> CHAMADA
                    </button>
                    <button onClick={() => mudarFase('votacao')} className={`p-2 rounded text-xs font-bold flex flex-col items-center gap-1 ${fase === 'votacao' ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                        <Play size={16} /> VOTAÇÃO
                    </button>
                </div>
            </section>
            ...
            <section className={fase === 'fechada' ? 'opacity-50 pointer-events-none' : ''}>
                ...
                <div className="space-y-2">
                    <input type="text" value={materiaTitulo} onChange={(e) => setMateriaTitulo(e.target.value)} className="w-full bg-black/30 border border-gray-600 rounded p-2 text-xs text-white outline-none" placeholder="Título" />
                    <textarea value={materiaTexto} onChange={(e) => setMateriaTexto(e.target.value)} className="w-full bg-black/30 border border-gray-600 rounded p-2 text-xs text-white outline-none resize-none h-20" placeholder="Texto..." />
                </div>
            </section>
            ...
            <section>
                <div className="flex justify-between items-end mb-2">
                    ...
                    {fase === 'votacao' && (
                        <button onClick={resetarVotos} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1 font-bold">
                    ...
                    {vereadores.map(ver => (
                        <div key={ver.id} className="bg-black/30 p-2 rounded flex flex-col gap-1 border border-gray-700/50">
                            <div className="flex justify-between items-center">
                                ...
                                <button onClick={() => togglePresenca(ver.id)} className={`text-[10px] px-2 py-0.5 rounded font-bold ${ver.presente ? 'bg-green-900/60 text-green-400 border border-green-800' : 'bg-gray-800 text-gray-600 border border-gray-700'}`}>
                                    {ver.presente ? 'PRESENTE' : 'AUSENTE'}
                                </button>
                            </div>

                            {ver.presente && fase === 'votacao' && (
                                <div className="grid grid-cols-3 gap-1 mt-1">
                                    <button onClick={() => computarVoto(ver.id, 'sim')} className={`p-1 rounded flex justify-center ${ver.voto === 'sim' ? 'bg-green-600 text-white' : 'bg-gray-800 text-gray-500'}`}><CheckCircle size={14} /></button>
                                    <button onClick={() => computarVoto(ver.id, 'nao')} className={`p-1 rounded flex justify-center ${ver.voto === 'nao' ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-500'}`}><XCircle size={14} /></button>
                                    <button onClick={() => computarVoto(ver.id, 'abstencao')} className={`p-1 rounded flex justify-center ${ver.voto === 'abstencao' ? 'bg-yellow-600 text-white' : 'bg-gray-800 text-gray-500'}`}><MinusCircle size={14} /></button>
                                </div>
                            )}
                        </div>
                    ))}
