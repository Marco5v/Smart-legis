...
const CardVereador: React.FC<CardVereadorProps> = ({ vereador, fase }) => {
  ...
  return (
    <motion.div
      layout
      animate={{ scale: vereador.voto ? [1, 1.05, 1] : 1 }}
      transition={{ duration: 0.3 }}
      className={`flex flex-col rounded-xl border-2 ${corBorda} bg-gray-900/80 backdrop-blur-sm transition-all duration-300 ${sombra} h-full overflow-hidden`}
    >
      <div className="flex-1 flex items-center px-3 gap-3 min-h-0 py-1">
        ...
        <div className="flex-1 min-w-0 h-full flex flex-col justify-center items-start text-left">
            <div className="mb-0.5 min-h-[14px] flex items-end w-full justify-start">
                {vereador.cargo ? (<span className="text-[9px] md:text-[10px] text-yellow-500 font-black uppercase tracking-wider leading-none">{vereador.cargo}</span>) : (<span className="text-[9px] md:text-[10px] opacity-0 select-none">.</span>)}
            </div>
            ...
            <div className="mt-0.5 flex w-full justify-start">
                <span className={`text-[9px] md:text-[10px] font-bold inline-block px-1.5 py-px rounded border ${vereador.presente ? 'bg-gray-800 border-gray-600 text-blue-300' : 'bg-gray-900 border-gray-800 text-gray-700'}`}>{vereador.partido}</span>
            </div>
        </div>
      </div>
...
const PanelPage: React.FC = () => {
  ...
  const materiaTitulo = session.currentProject?.title || 'AGUARDANDO PAUTA';
  const materiaTexto = session.currentProject?.description || 'Nenhuma matéria em discussão no momento.';
